...
procedure Divide
  (Dividend  : in  Very_Long;
   ...
   -- two digits. The case when the divisor has only one digit must be handled separately.
   subtype Divisor_Digits_Count_Type is Digit_Index_Type range 2 .. Divisor.Length;
   ...
   subtype Quotient_Digits_Count_Type is
     Digit_Index_Type range Divisor.Length .. 2*Divisor.Length - 2;
   ...
   -- Refer to the reference on multiprecision division to understand the names here.
   ...
   -- Shifts Number to the left by Distance bits.
   -- The final overflow is dropped (should be zero when used as below).
   ...
   -- Shifts Number to the right by Distance bits.
   -- The final overflow is dropped (should be zero when used as below).
   ...
   procedure D3(J : in Digit_Index_Type)
     with
       Global => (Input => (N, U, V), Proof_In => Dividend, Output => Q_Hat),
   ...
   begin
      ...
      if Current_Borrow /= 0 then
         ...
         for Divisor_Index in Digit_Index_Type range 1 .. N loop
            ...
            U.Long_Digits(J + Divisor_Index - 1) := Octet(Sum and 16#00FF#);
            Carry := Shift_Right(Sum, 8);
         end loop;
         U.Long_Digits(J + N) := U.Long_Digits(J + N) + Octet(Carry and 16#00FF#);
      end if;
   end D5;
