...
package Bit_Files is
   ...
   procedure Put (File : in out File_Type; Bit : Bit_Type);

   --  Read a bit from the specified bit file.
   procedure Get (File : in out File_Type; Bit : out Bit_Type);

   --  Close the specified bit file. If the file was open for output, the final bits are
   --  flushed. The last byte is zero-filled if necessary.
   --
